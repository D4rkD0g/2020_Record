...
def maybe_process_file(f, patch_infos):
    ...
    global num_links_skipped, num_dotdirs_skipped,exdir
    if os.path.isdir(f):
        if (not allowlink) and os.path.islink(f):
            if not quiet:
        ...
        for dir_entry in os.listdir(f):
            if dir_entry in exdir:continue
...
def process_options():
    ...
    try:
        ...
        optlist, args = getopt.getopt(sys.argv[1:], "x:ce:m:nih?CSDQHIFP:", [
            "exclude=", "context", "minlevel=", "immediate", "inputs", "input", "nolink",
